...
* **[Web 應用程式防火牆](application-gateway-webapplicationfirewall-overview.md)** - Azure 應用程式閘道中的 Web 應用程式防火牆 (WAF) 可保護 Web 應用程式，不致遭受常見的 Web 型攻擊，例如 資料隱碼（SQL Injection）攻擊、跨網站指令碼（XSS）攻擊和工作階段（Session）攔截。
...
* 保護 Web 應用程式不致遭受常見的 Web 型攻擊，例如資料隱碼（SQL Injection）攻擊、跨網站指令碼（XSS）攻擊和工作階段（Session）攔截。
